...
<p><strong>因果掩码还有一个训练时的妙用，值得点破。</strong>训练时我们手里有<strong>完整的句子</strong>，可以把它整段一次喂进模型；
因果掩码保证第 i 个位置<strong>只能看到 &lt;= i 的词</strong>，于是"用前文预测下一个词"这件事，在<strong>每一个位置上同时成立</strong>——
一次前向就拿到了全句每个位置的"下一个词"预测，可以并行地和真实的下一个词比对、算损失。这种"<strong>把答案也一起喂进去、
但靠掩码挡住未来</strong>"的训练方式叫 teacher forcing，让训练极其高效。有意思的是：<strong>这和推理时的 prefill 是同一套机制</strong>——
prompt 里每个 token 都已知，于是能像训练那样一次并行算完，只不过推理时我们只关心最后一个位置的输出。</p>
...
<p><strong>The causal mask also has a neat training use worth spelling out.</strong> During training we have the <strong>full sentence</strong> and can
feed it in all at once; the causal mask guarantees position i sees only tokens <strong>&lt;= i</strong>, so "predict the next word from the prefix" holds
<strong>at every position simultaneously</strong> - one forward pass yields a "next word" prediction at every position, compared in parallel against the
real next words to compute the loss. This "<strong>feed the answers in too, but block the future with a mask</strong>" training is called teacher forcing,
and it is extremely efficient. Tellingly, <strong>this is the very same mechanism as prefill at inference time</strong> - every prompt token is known, so it
can be computed in parallel like training, except at inference we only care about the last position's output.</p>
...
<p>抽象归抽象，张量到底用来装什么？在 llama.cpp 里，<strong>模型的每个权重矩阵</strong>是张量（比如词嵌入表是一个
<span class="mono">[n_embd, n_vocab]</span> 的大张量、每层的注意力与 FFN 权重也都是张量），<strong>前向过程中流动的激活值</strong>是张量，
连 <strong>KV cache</strong> 里存的那些 K 和 V 也是张量。可以说，一次推理从头到尾，就是<strong>一堆张量按计算图被算来算去</strong>。
正因为如此，把"张量"这个抽象设计得又轻又灵活，对整个引擎的效率至关重要。</p>
...
<p>张量从哪来？在 ggml 里，你先开一个 <span class="mono">ggml_context</span>（一个内存池），再用 <span class="mono">ggml_new_tensor_2d</span>
这类函数在池子里"<strong>登记</strong>"一个张量——它会按 type 和 ne 算好需要多少字节、把 nb 填好。值得注意的是：<strong>建张量时通常并不立刻
搬运那一大块数据</strong>，很多时候只是先把"形状说明书"建好（这正配合了 ggml 先建图、后执行的风格），真正的内存分配与计算留到后面统一来做。
第三部分会专门讲 <span class="mono">ggml_context</span> 与这套"先描述、后执行"的内存管理。</p>
...
<p>像 reshape、转置、切片、广播这类操作，ggml 大多用<strong>视图（view）</strong>实现：新张量复用<strong>同一块 data</strong>，只是带上不同的
<span class="mono">ne</span>/<span class="mono">nb</span> 和偏移，并用 <span class="mono">view_src</span> 记住"我是谁的视图"。好处显而易见：<strong>零拷贝、省内存、还快</strong>。
代价是：视图常常变得<strong>不连续</strong>（<span class="mono">ggml_is_contiguous</span> 为假）——比如转置之后，沿 <span class="mono">ne[0]</span>
方向走，在内存里就不再是挨着的了。有些算子要求输入必须连续，这时要先用 <span class="mono">ggml_cont</span> 把它"压实"成一块新的连续内存
（注意：<strong>这一步才真的发生拷贝</strong>）。所以你会在 ggml 代码里看到不少 <span class="mono">ggml_cont</span> 的调用，它们就是在"连续性"和"零拷贝"
之间做权衡。</p>
...
靠的全是视图，几乎不产生额外拷贝。</p>
<p>再比如<strong>切片</strong>：想从一大块张量里取出"第 k 层"或"第 h 个注意力头"那一小块，ggml 通常也不复制，而是<strong>算好一个起始偏移</strong>、
配上裁剪过的 ne/nb，返回一个指回原数据的视图。所以在 ggml 代码里你会发现，"<strong>取一部分</strong>"和"<strong>换个形状看</strong>"在底层往往是
同一种廉价操作——这套以视图为中心的玩法，是读懂后面计算图代码的一把钥匙。</p>
...
<p>最后回头看 <span class="mono">op</span> 和 <span class="mono">src</span> 这两个字段，它们让张量有了"<strong>双重身份</strong>"：既是<strong>一块数据</strong>，
又是<strong>计算图里的一个节点</strong>。当你写下 <span class="mono">c = ggml_mul_mat(ctx, a, b)</span>，ggml 并不立刻算矩阵乘，而是新建一个张量
<span class="mono">c</span>，把它的 <span class="mono">op</span> 记成"矩阵乘"、<span class="mono">src[0]/src[1]</span> 指向 <span class="mono">a</span> 和
<span class="mono">b</span>。于是整张计算图，其实就是<strong>张量们靠 src 互相牵着手连成的一张网</strong>；等图建完，再交给后端按这张网的顺序逐个算过去。
理解了张量这层"图节点"身份，你就握住了第三部分 ggml 引擎的钥匙。</p>
...
<p>给个实战提示：在 ggml 里写代码，<strong>最常见的报错就是形状对不上</strong>——矩阵乘要求左右两个张量在相乘的那一维上元素数相等，
转置、reshape 之后维度顺序变了，很容易把该对齐的维弄错。养成<strong>随手在脑子里写出每个张量 ne</strong> 的习惯（甚至用 <span class="mono">name</span>
标注、打印出来核对），能帮你省下大量调试时间。这也是为什么这一课要把 ne/nb 讲得这么细：<strong>形状是 ggml 编程的"语法"</strong>，语法错了，后面什么都跑不起来。</p>
...
<p>Abstractions aside, what do tensors actually hold? In llama.cpp, <strong>every weight matrix</strong> is a tensor (the embedding table is a big
<span class="mono">[n_embd, n_vocab]</span> tensor; each layer's attention and FFN weights are tensors too), the <strong>activations flowing through the forward
pass</strong> are tensors, and even the K and V stored in the <strong>KV cache</strong> are tensors. An entire inference, start to finish, is just <strong>a pile
of tensors being computed along a graph</strong>. That is why making the "tensor" abstraction light and flexible is so crucial to the whole engine's efficiency.</p>
...
<p>Where do tensors come from? In ggml you first open a <span class="mono">ggml_context</span> (a memory pool), then use functions like
<span class="mono">ggml_new_tensor_2d</span> to "<strong>register</strong>" a tensor in the pool - it computes the needed bytes from type and ne, and fills in nb.
Notably, <strong>creating a tensor usually does not immediately move that big block of data</strong>; often it just sets up the "shape manual" (matching ggml's
build-graph-then-execute style), leaving the real allocation and computation for a unified later pass. Part 3 covers <span class="mono">ggml_context</span> and this
"describe first, execute later" memory management.</p>
...
<p>Operations like reshape, transpose, slice, and broadcast are mostly implemented as <strong>views</strong> in ggml: the new tensor reuses the <strong>same
data</strong>, just with different <span class="mono">ne</span>/<span class="mono">nb</span> and an offset, recording its origin in <span class="mono">view_src</span>.
The upside is obvious: <strong>zero-copy, memory-saving, and fast</strong>. The cost: views often become <strong>non-contiguous</strong>
(<span class="mono">ggml_is_contiguous</span> is false) - e.g. after a transpose, walking along <span class="mono">ne[0]</span> is no longer adjacent in memory.
Some operators require contiguous inputs, in which case you first call <span class="mono">ggml_cont</span> to "compact" it into a fresh contiguous block
(note: <strong>this step does copy</strong>). That is why you see many <span class="mono">ggml_cont</span> calls in ggml code - balancing "contiguity" against "zero-copy".</p>
...
<p>Or <strong>slicing</strong>: to pull out "layer k" or "head h" from a big tensor, ggml usually does not copy either - it <strong>computes a starting
offset</strong>, pairs it with trimmed ne/nb, and returns a view pointing back at the original data. So in ggml code "<strong>take a part</strong>" and "<strong>see
it in a different shape</strong>" are often the same cheap operation underneath - this view-centric style is a key to reading the compute-graph code later.</p>
...
<p>Finally, back to <span class="mono">op</span> and <span class="mono">src</span> - they give a tensor a "<strong>dual identity</strong>": both <strong>a block of
data</strong> and <strong>a node in the compute graph</strong>. When you write <span class="mono">c = ggml_mul_mat(ctx, a, b)</span>, ggml does not compute the matmul
immediately; it creates a new tensor <span class="mono">c</span>, records its <span class="mono">op</span> as "matmul" and points <span class="mono">src[0]/src[1]</span> at
<span class="mono">a</span> and <span class="mono">b</span>. So the whole compute graph is just <strong>tensors holding hands via src into a web</strong>; once built, it is
handed to the backend to compute node by node in order. Grasp this "graph node" identity of tensors and you hold the key to Part 3's ggml engine.</p>
...
<p>A practical tip: the <strong>most common error</strong> writing ggml code is a shape mismatch - matmul requires the two tensors to have equal element counts
on the multiplied dimension, and after transpose/reshape the dim order changes, so it is easy to misalign. Get into the habit of <strong>writing out each tensor's
ne in your head</strong> (or tagging with <span class="mono">name</span> and printing to check) - it saves a lot of debugging time. That is why this lesson belabors
ne/nb: <strong>shape is the "grammar" of ggml programming</strong>; get the grammar wrong and nothing downstream runs.</p>
