...
<h2>怎么用这份速查 + 收个尾</h2>
<p>用法很简单：把这一页加进收藏。当你读源码或调 bug 撞见一个名词，先来术语表扫一眼"是什么、在哪定义"，需要深入就点链接回到那一课的现场；想理清几个概念的关系，就回头看那张依赖图。它不替代任何一课，只是让你不必为了一个名词把整门课重翻一遍——这正是"速查"二字的全部意义。 也别忘了浏览器自带的页内搜索（Ctrl/Cmd+F）：在这一页直接搜一个英文符号名，往往比你回想"它在第几课"还快。</p>
<p>还要提醒一句：这二十来个词只是<strong>骨架</strong>，不是全部。书里还讲过不少同样重要、但更专门的概念——比如聊天模板（L22）、语法约束解码（L23）、<span class="mono">llama-server</span> 的连续批处理（L28）、量化工具与评测（L29/L30）。它们没进这张速查表，只是因为速查讲究"少而准"；真要用到，顺着对应课的标题就能找回去。换句话说，这张表是一份<strong>最常用词的索引</strong>，而整门课才是完整的词典。 用熟了你会发现，查得越少、记得越牢——好的速查表，最终是为了让你不再需要它。</p>
