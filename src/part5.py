...
<p style="color:var(--muted)">本课只做<strong>架构总览</strong>——把请求从进门到出门的主干道走通，把 slot 与连续批处理这两个核心概念讲清。更深的调度取舍（prefill 与 decode 如何交错、batch 容量、抢占与公平）超出本课总览的范围，这里点到为止。</p>
...
<details class="accordion">
  ...
  <div class="acc-body">
    <p>The number of slots is fixed (<span class="mono">--parallel N</span>). If all N slots are busy and a new request arrives, it is not dropped but <strong>queued to wait</strong>: it stays in the <span class="mono">server_queue</span> (the implementation has a "deferred tasks" queue) until some slot finishes generating and returns to <span class="mono">IDLE</span>, then the scheduler takes it out and assigns it. So <span class="mono">--parallel</span> is a trade-off: larger means more concurrent requests, but each slot's KV context (<span class="mono">n_ctx_slot</span>) and compute are thinner; smaller is the opposite. How to weigh it depends on your VRAM and load - exactly a deeper layer of scheduling trade-off. A handy rule of thumb: estimate from "how much context one request needs" how much KV each slot must reserve, then divide remaining VRAM by it, and that is roughly the ceiling for N - beyond it you either blow VRAM or squeeze each one's context too thin.</p>
  </div>
</details>
<details class="accordion">
...
LESSON_29 = {
    "zh": r"""
<p class="lead" style="font-size:1.06rem;color:var(--muted);margin-top:-.6rem">
前面的 L06 和 L12 已经讲透了量化的"原理"——为什么几个比特就能近似一个浮点数、各种格式的字节又是怎么排布的。这一课换个角度，讲"<strong>怎么用工具真把模型压小</strong>"：一行 <span class="mono">llama-quantize</span> 命令，就能把一个十几 GB 的 fp16 模型变成三五 GB 的 Q4_K_M；再配上 imatrix（重要性矩阵），同样的比特数还能把掉下去的质量再拉回来一截。
</p>
<p style="color:var(--muted);margin-top:.4rem">换句话说，L06/L12 是"懂原理"，这一课是"会操作"：知道每个旗标在调什么、不同档位是怎么取舍体积与质量的、以及 imatrix 这把"质量回血"的钥匙到底怎么用。量化是让大模型能在普通显卡、甚至纯 CPU 上跑起来的关键一步，而这一课就是教你亲手把它完成。</p>
...
<div class="card spark">
  <div class="tag">💡 设计洞察</div>
  量化工具这一课，藏着一个反复出现的工程智慧：<strong>面对有限的预算，与其平均分配，不如按重要性分配</strong>。imatrix 不增加一个比特，只是把同样的比特花得更聪明——这和缓存把热数据放近、调度器把算力给关键任务，是同一种思路。它也提醒我们：很多"免费的午餐"其实来自"先花点力气测量、再据此优化"。生成 imatrix 要先跑一遍校准文本（花点时间），换来的却是同等体积下更好的质量（长期受益）。从 L06/L12 的"原理"到这一课的"工具"，你现在不仅知道量化是什么，还知道怎么把它用到最好——下一课，我们就用困惑度这把尺子，亲手量一量量化到底损失了多少。再往大里说，这种"先测量、再按重要性分配"的思路，在计算机科学里到处都是：JIT 编译器先看哪些代码热、再重点优化它；数据库先统计哪些查询频繁、再为它们建索引。它们和 imatrix 共享同一条信念——<strong>与其凭空猜，不如用真实运行数据说话</strong>。把这条信念带在身上，你以后遇到任何"资源有限、又想要最好效果"的问题，都会本能地先问一句：能不能先测一测，看看力气该往哪儿使？这，比记住任何一个量化档位的名字都更有用。
</div>
""",
    "en": r"""
<p class="lead" style="font-size:1.06rem;color:var(--muted);margin-top:-.6rem">
...
<p style="color:var(--muted);margin-top:.4rem">In other words, L06/L12 is "understand the principle", this lesson is "operate the tool": knowing what each flag tunes, how different levels trade size against quality, and how to use imatrix, that "quality-restoring" key. Quantization is the crucial step that lets big models run on ordinary GPUs or even pure CPU, and this lesson teaches you to do it by hand.</p>
...
<p>所以 PPL 本质上就是把"模型对一整段真实文本的平均惊讶程度"压缩成一个数。同一段文本，你拿 fp16 原模型和它的 Q4 量化版各算一次 PPL，两个数一比，就知道量化到底让模型"变笨"了多少——L29 指给你看的那张 <span class="mono">quantize.cpp</span> 档位表里"困惑度代价"那一列，正是这么一段段文本测出来的。除了这种语言建模困惑度，perplexity 工具还支持 HellaSwag、Winogrande 等任务级评测（按 <span class="mono">n_chunk</span> 切块、算多选准确率），从另一个角度看模型好坏。这里还藏着一个常被忽略的前提：算 PPL 必须用模型<strong>训练时没见过</strong>的文本才有意义——拿训练数据来算，模型"背过答案"，PPL 会假性偏低，量不出真实的泛化能力。</p>
