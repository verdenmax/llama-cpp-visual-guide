...
<strong>KV cache</strong> 把先前每个 token 算出的键/值缓存了下来。所以第一次吃下整段 prompt（<strong>prefill</strong>）相对慢，
...
history - thanks to the <strong>KV cache</strong>, which stores the keys/values already computed for every prior token. That is why digesting the whole
prompt the first time (<strong>prefill</strong>) is relatively slow, while generating word by word afterward (<strong>decode</strong>) is fast, each step
...
<p>这 7 步里有几处容易一带而过、其实值得多看一眼。<strong>第 1 步"分词"切出的是 subword（子词），既不是按单字、也不是按整词</strong>：llama.cpp 的分词器走的是 BPE / SPM / WordPiece 这类<strong>字节级子词</strong>算法（见 <span class="mono">include/llama.h</span> 的 <span class="mono">LLAMA_VOCAB_TYPE_SPM / _BPE / _WPM</span>），一个常见英文单词可能正好是一个 token，而生僻词或一个汉字往往被拆成<strong>好几个子词片</strong>。这也正好解释了第 6 步为什么要用 <span class="mono">llama_token_to_piece</span> 把 token 还原成"<strong>词片</strong>"——多个片拼起来才是一个完整的词或汉字，所以你看到的输出是"<strong>一个 token 一个 token 地往外蹦</strong>"，而不是规规矩矩一个字一个字地出。换句话说，<strong>token 并不等于"词"</strong>，它只是模型词表里的一个最小单位。这也带来一个很实际的后果：<strong>token 数和字符数往往对不上</strong>——同一段话，中文、英文、代码切出的 token 数可能差很多，而上下文窗口、计费、速度都是按 <strong>token</strong> 算的，不是按字数算的。</p>
