...
3. `<h2>` 顶层目录速览 + `<table class="t">`（两列：目录 / 作用）。**用下列已核实的行**（en 自行翻译，保持同序同表）：
   ...
   - `tools/` — **可执行程序**（用二进制名）：`llama-cli` · `llama-server` · `llama-quantize` · `llama-mtmd-cli`（多模态）· `llama-perplexity` · `llama-bench` …
   - `examples/` — 小型示例程序（如 `simple`）
   - `gguf-py/` — **Python 的 GGUF 读写库**
   - `convert_hf_to_gguf.py` 等 — **Python 转换脚本**（共 4 个 `convert_*.py`，主力是 HuggingFace -> GGUF；另含 LoRA、旧版 GGML、tokenizer 维护）
