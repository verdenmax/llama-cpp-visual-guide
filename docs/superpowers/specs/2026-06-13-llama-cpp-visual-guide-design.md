...
- **教学卡片**：
  ...
  - ⚠️ **坑 / 注意**（warn，按需）。
- **图（重点强化）**：尽量多用结构图 / 数据流图 / 分层架构 / 时序步骤（flow / vflow / layers / 自绘 SVG 或 HTML+CSS）。
- **代码（重点强化）**：**伪代码** + **从源码简化的真实片段**（带高亮），讲清"为什么这么写"。
- **折叠深挖**（accordion）：可选的深入细节，新手可跳过。
- **末尾自测**（quiz）：2-4 题，点开看解析。
- **底部导航**：上一课 / 下一课。
...
1. **多图**：能画图就画图。每课尽量至少 1-2 张结构/流程/分层图；宏观课要有"整体结构图"。
2. **多代码**：以**伪代码**讲清思路，再给**从源码简化的真实片段**对照（标注来源文件 + 符号）。
3. **尽量详细**：每部分内容宁多勿少，把"是什么 / 为什么这么写 / 还有什么替代方案"讲透。
